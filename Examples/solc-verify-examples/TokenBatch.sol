...
contract Token {
    ...
    function batchTransfer(address payable[] memory receivers, uint value) public {
        require(0 < receivers.length && receivers.length <= 10, "");
        uint amount = receivers.length.mul(value);

        require(balances[msg.sender] >= amount, "Insufiecient funds");
