...
compiled_contracts = get_contract_abi_bin("./PhishingTxOrigin.sol")

for ctr in compiled_contracts:

    if(ctr == 'PhishingTxOrigin.sol:Attack'):
        attack_abi_bin = (compiled_contracts[ctr]['abi'], compiled_contracts[ctr]['bin'])

    if(ctr == 'PhishingTxOrigin.sol:Wallet'):
...
attack = c.ImplicitContract(attack)

#### SWITH TO ACCOUNT 0 AND PERFORM PHISHING ATTACK
w3.eth.default_account = accounts[0]
try:
    attack.attack()
...
print(f"\nPHISHING ATTACK WAS SUCCEED. ATTACKER WAS ABLE TO STEAL {stolen_ether} ETHER.. NEVER USE TX.ORIGIN TO AUTHENTICATE!!!")
